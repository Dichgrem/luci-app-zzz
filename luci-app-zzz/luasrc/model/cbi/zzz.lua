...
local m = Map("zzz", "ZZZ 802.1x 认证客户端", "配置使用 zzz 客户端进行网络访问的 802.1x 认证")

m.on_after_commit = function(self)
	sys.call("service zzz restart >/dev/null 2>&1 &")
end

local s = m:section(TypedSection, "auth", "认证设置")
s.anonymous = true
s.addremove = false
...
o.rawhtml = true
o.cfgvalue = function()
	local running = sys.call("service zzz status >/dev/null 2>&1 && pgrep zzz >/dev/null") == 0
	if running then
...
schedule_time.datatype = "and(minlength(5),maxlength(5))"
...
auto_start.write = function(self, section, value)
	...
	if value == "1" then
		...
		sys.call(
			string.format(
				"echo '%s %s * * 1,2,3,4,5 service zzz start # zzz auto start' >> %s",
				minute,
				hour,
				temp_cron
			)
		...
		sys.call("service cron enable && service cron start")
	else
		sys.call("crontab -l 2>/dev/null > " .. temp_cron)
		sys.call("sed -i '/S99zzz/d' " .. temp_cron)
		sys.call("sed -i '/# zzz auto/d' " .. temp_cron)
		sys.call("crontab " .. temp_cron .. " 2>/dev/null && rm -f " .. temp_cron)
		sys.call("service cron restart")
	end
end
...
timer_status_display.cfgvalue = function()
	local cron_output = sys.exec("crontab -l 2>/dev/null | grep -E 'zzz|S99zzz' || echo '未设置'")
		...
		if cron_line then
			...
			if minute and hour then
				...
				return string.format(
					"<span style='color:green;font-weight:bold'>✔ 已启用 (每周一至周五 %s 自动启动)</span>",
					time_str
				)
			end
		end
